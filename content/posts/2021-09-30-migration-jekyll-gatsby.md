...
Les sites statiques te proposent souvent de rédiger le contenu sous forme de [Markdown](https://www.markdownguide.org/cheat-sheet/) qui est un language de balisage très accessible.

Un site statique possède beaucoup d'avantages. **L'avantage principal est qu'il n'y a pas de base de données ni de language interprété côté serveur**. Donc

1. **La sécurité est renforcé**. La pluparts des failles de sécurité reposent sur le language interprété côté serveur.
2. **Le coût du serveur est réduit**. J'utilise un simple [VPS chez OVH](https://www.ovhcloud.com/fr/vps/) à 3€/mois qui hébérge aussi d'autres sites statiques. Et avant de passer chez OVH, j'utilisait [un Raspberry PI branché à ma box SFR](2017-10-11-installer-bridge-sfr-box-4k)
3. **Les performances sont excelentes** car le serveur n'a pas besoin d'interpreter un language ni de faire des requêtes en base de données.
4. **Le SEO** qui dépend directement de la performance du site et notament du temps de réponse

Je peux te prouver le résultat avec cette commande cURL qui montre le temps de réponse de mon site comparé à <https://wordpress.com/> (qui est site fonctionannt avec un CMS et une base de données):
...
On voit donc que mon site répond en 10ms alors que wordpress en preque une demie seconde. Donc un site statique répond **45 fois plus vite qu'un CMS classique** [^1].
...
Bref, si ton contenu n'est pas voué a évoluer tous les jours et que tu à la possibilité de l'écrire da un générateur de site statique est la solution.

## Jekyll

Etant un fan du language Ruby, j'avais initialement choisis [Jekyll](https://jekyllrb.com/) pour générer mon site. Jekyll est un des outils les plus populaire pour générer un site statique rapidement. C'est d'ailleur l'outil utilisé pour [Github Pages](https://guides.github.com/features/pages/). C'est un outil que je recommande pour débuter car il est très facile à prendre en mains, même sans connaissance du language Ruby.
...
Je suis habitué aux **frameworks frontend**. Je ne sais donc plus faire du HTML/CSS "à l'ancienne". Je me suis habitué à l'architecture par composants. La philosophie de Jekyll à utiliser du HTML/CSS natif me ralentissai. Aussi, je travaille majoritairement avec l'écosystème JavaScript. **Je n'aime pas écrire du JavaScript "à l'ancienne"** (a.k.a `document.querySelector` et autres joyeusetés). C'est totalement objectif mais je voulais me tourner vers une façon de faire moderne.

Mon second point est que **Jekyll devient plus compliqué lorsqu'on souhaite sortir des clous**. Voici par exemple quelques fonctionalités qu'il m'était complqiué de développer:
...
Mon troisème point, est que je cherchais à m'experimenter sur de nouvelles technologies. Si tu es développeur, tu sais tout comme moi que le web bouge et très vite. Les technologies d'il y a quelques années ne sont pas obsolète.

Pour conclure, **Jekyll faisait très bien le travail** et reste un outil très adapté. Il ne **me** correspondais juste plus.

## Quel générateur de site statique choisisr?
...
3. la taille de ton site. Certains outils sont plus adapaté au très gros sites.
...
Les frameworks web-moderne comme React, Vue.js ou Angular sont de superbes outils pour des applications web mais ils peuvent générer des sites nécéssitant de télécharger et d'éxecuter **plusieurs méga-octets de JavaScript**. Je trouve que c'est un non-sens pour un simple blog. Les librairies qu'on charge sur un site représentent une grande quantité de données.

![Les librairies JavaScript pèse lourd](./images/node-modules-meme.png)

[^2]: C'est d'ailleur une des raison pour laquelle j'ai choisis de ne pas intégrer de module de tracking et que je [m'appuie sur les logs](./2019-11-07-lire-les-logs-avec-go-access.md) pour analyser les pages les plus consultées.

Gastby répond parfaitement à cela car il effectue tout un tas d'optimisations par defaut:
...
Les frameworks JavaScript modernes patissent de ce problème car sans JavaScript, le contenu HTML ne peut pas être contruit. Voici un exemple avec Twitter:
...
J'ai développé de zero le site avec Gatsby afin de le prendre en main. Cela a été le plus gros du travail. Néanmoins, Il est possible d'utiliser [un templates de blog pour Gatsby](https://www.gatsbyjs.com/starters/gatsbyjs/gatsby-starter-blog/). Cela évite la partie développement pur.
...
- d'uniformiser l'en-tête _frontmater_ des fichiers Markdown. Tous les articles doivent comporter les mêmes clés commes `date`, `title`, etc...
...
Pour envoyer ces status, il est possible d'utiliser un fichier `.htaccess` avec Apache.
...
Afin de mettre en place le fichier `.htaccess`, j'ai choisis d'utiliser le plugin [`gatsby-plugin-htaccess`](https://www.gatsbyjs.com/plugins/gatsby-plugin-htaccess/). Ge j'ai configuré rapidement comme ceci
...
Dans mon cas, n'utilisant que du Markdown, **la migration de Jekyll à Gatsby n'était pas nécéssaire**. Si tu utilise aussi Jekyll et que cela te conviens, reste avec Jekyll. Dans mon cas, la motivation de cette migration était vraiment motivée par apprendre de nouveaux outils.

Développer avec Gatsby ma demandé beaucoup d'efforts car **c'est un outils beacoup plus complexe**. En revanche, je ne regrette pas le temps que j'y ait investi. Je me servirai très certainement de cet outils lors d'un futur projet.

Gatsby m'a aussi offert la possibilité d'utiliser des technologies modernes tout en me donnant les performances que j'attendais. Ca a été vraiment plaisant de pouvoir utiliser React au seins de mon petit projet!
